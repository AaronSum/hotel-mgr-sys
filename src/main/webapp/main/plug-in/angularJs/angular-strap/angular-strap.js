    ...
    this.$get = ["$window", "$document", "$rootScope", "$tooltip", "$timeout", function($window, $document, $rootScope, $tooltip, $timeout) {
      ...
      function SelectFactory(element, controller, config) {
        ...
        $select.$getIndex = function(value) {
          ...
          var comparator;
          if (angular.isObject(value) && options.compareKey) {
            comparator = function(lV,rV){
              return lV[options.compareKey] === rV[options.compareKey];
            };
          } else {
            comparator = function(lV,rV){
              return lV === rV;
          ...
          for(i = l; i--;) {
            if(comparator(scope.$matches[i].value, value)) break;
          }
          if(i < 0) return;
          return i;
        };
        ...
        $select.show = function() {
          ...
          $timeout(function() {
            ...
            // var maxHei = window.innerHeight - clickSrcEvent.clientY + clickSrcEvent.offsetY - clickSrcEvent.target.clientHeight - 50;
            // $select.$element.css("max-height", maxHei);
            if(options.keyboard) {
  ...
  .directive('bsSelect', ["$window", "$parse", "$q", "$select", "$parseOptions", function($window, $parse, $q, $select, $parseOptions) {
    ...
    return {
      ...
      link: function postLink(scope, element, attr, controller) {
        ...
        angular.forEach(['placement', 'container', 'delay', 'trigger', 'keyboard', 'html', 'animation', 'template', 'placeholder', 'multiple', 'allNoneButtons', 'maxLength', 'maxLengthHtml', 'allText', 'noneText', 'iconCheckmark', 'autoClose', 'id', 'sort', 'caretHtml', 'compareKey'], function(key) {
          if(angular.isDefined(attr[key])) options[key] = attr[key];
        });
