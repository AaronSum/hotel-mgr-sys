...
.controller("loginController", ["$rootScope", "$scope", "$http", function($rootScope, $scope, $http) {
	...
	//免费开店url
	$scope.regLogin = contentPath + "/main/reg/reglogin.jsp";
	// pms登录
	$scope.pmsLogin = contentPath + "/loginJin/main";
	...
	$scope.isExitLoginUser = function() {
		...
		$http({
		    ...
		    data: $.param({
		    	"j_username": jQuery("#loginName").val().trim(),
				"j_password": jQuery("#password").val()
		    }),
		    headers: {'Content-Type': 'application/x-www-form-urlencoded'}
		})
		.success(function(data){
			$http.post(contentPath + "/login/isExit", {
