...
public class HotelRuleWorker{
	
	@Autowired
	private HmsHotelRuleService hmsHotelRuleService;
	
	private static final Logger logger = Logger.getLogger(HotelRuleWorker.class);
	protected ThreadPoolExecutor threadPool;
	/*
	 * 线程池属性
	 * 线程池保存线程大小 corePoolSize
	 * 保存任务的队列大小 queueCapacity
	 * 线程池最大规模 maximumPoolSize
	 * 超过规模线程最大存活时间 keepAliveTime
	 * 
	...
	public void doTask() throws Exception {
		//获取有效数据
		List<HMSHotelRuleModel> rules = hmsHotelRuleService.getInitTask();
		//是否需要执行task
		if(CollectionUtils.isEmpty(rules)){
			logger.info("没有数据，不需要更新");
			return;
		}
		...
		for (final HMSHotelRuleModel rule: rules) {
			threadPool.execute(new Runnable() {
				...
				public void run() {
					try {
						//同步酒店信息
						hmsHotelRuleService.syncHotelRule(rule);
					} finally {
