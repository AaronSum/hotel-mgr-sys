 ...
 *
 */
public interface BRuleHmsOtsOrderMapper{
	 ...
	 * @return
	 */
	@Select("SELECT t.*,l.allcost,l.realotagive FROM (select * from b_otaorder where ordertype=#{ordertype} and id in (${ids}))t,p_pay p,p_orderlog l WHERE p.id = l.payid AND p.orderid = t.id order by t.id desc")
	List<HmsOtaOrderModel> findHmsOrdersByOrderIds(@Param("ordertype")int ordertype,@Param("ids") String ids);
	/**
	 * 获取hms订单信息
	 * @param pageNum 页码
	 * @param pageSize 页面数据条数
	 ...
	 * @return hms订单列表
	 */
	@Select("select * from b_otaorder where ordertype=${type} AND Hotelid = #{Hotelid} and Createtime >= #{Begintime} and Createtime <="
			+ " #{endTime}")
	List<HmsOtaOrderModel> findTodayHmsOrders(@Param("type") int type,@Param("Hotelid") long hotelId, @Param("Begintime") String beginTime, 
